...
InputBox::InputBox() {
  /* inputBox */
  inputBox.setSize(sf::Vector2f(1500, 200));
  ...
  inputBoxTxt.setCharacterSize(inputBox.getGlobalBounds().height * 0.75);
  inputBoxTxt.setFillColor(sf::Color::White);
  inputBoxTxt.setString("Menu");
  inputBoxCenterTxt();
}

void InputBox::inputBoxCenterTxt() {
  float x = inputBox.getPosition().x + (inputBox.getGlobalBounds().width - inputBoxTxt.getGlobalBounds().width) / 2;
  float y = inputBox.getPosition().y;
